...
| **Elad** | `EladNa1` | **Live deployment + real-time + scale** — the **live Azure deploy** (provision the VM + its GitHub secrets, make the GHCR packages public, UptimeRobot monitor, run the deploy demo — the pipeline *code* is in the repo, see Lior) + the test-runner service, the **Forum real-time layer** (notifications / DM / media), rate-limit (flask-limiter), stress / cross-container tests | [PERSON3.md](PERSON3.md) |
