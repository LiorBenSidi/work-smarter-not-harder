...
| 2 | ✅ | AI / scope | `PROPOSAL §5` (v1) named **5** classes incl. **"Injury Risk"**; PMData has **no injury label** and ~3 natural classes (`ai/README.md:24-26`). **Resolved 2026-07-20:** the TA's [`GUIDELINES.md`](GUIDELINES.md) (`WSNH_Guidelines.pdf`) is the source of truth and permits notified feature changes; the team's v2 proposal generalized the class set, and the shipped **3-class RF (Rest/Moderate/Ready)** satisfies it — **no 5th class owed, nothing pending.** | *(historical)* Lowest-risk option had been to keep "Injury Risk" as a rule-based label; not needed under the v2 spec. |
...
| 7 | 🟢 | Deploy (+10) | CI gate + the **deploy job now exist** in `ci.yml` (GHCR build/push → SSH-deploy-to-Azure → Caddy HTTPS + auto-rollback), gated on `vars.SSH_HOST != '' && vars.DEPLOY_ENABLED == 'true'`; the remaining steps are the **live VM go-live** (Elad) + wiring the stress tests (they need a running stack). | Built (was "next phase"). The only-`web`-exposed + env-config architecture supported the clean deploy job on green `main`, as designed. |
...
|---|---|---|---|
| Whole app F1–F9 on Docker | 80 | **Yes** — containers, contracts, data model map to every feature | Finding 2 (Injury-Risk) + the F6+F7 merge — **resolved** (settled by the TA's guidelines + the v2 spec; not pending) |
