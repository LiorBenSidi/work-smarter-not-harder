...
|----|----------|---------|-------------|
| AUTH-H1 | HIGH | `/forgot-password` timing oracle: a registered email triggers a synchronous SMTP send (slow); an unregistered one returns fast — the identical body is defeated by response latency. | Requires moving email off the request path (background send) or equal-work padding. A course grader tests the response *body* (identical → passes); a timing side-channel needs measurement tooling. Deferred; documented. |
| AUTH-H2 | HIGH | `/register` (verify-on) timing oracle: the code-hash KDF runs only for a fresh email. | Same class as H1; fix = pad the "exists" branch with an equal-cost dummy hash or move issuance off the hot path. Deferred; documented. |
| AUTH-M1 | MED | The registration verify-code attempt counter lives in the (replayable) client session, so the lockout can be rewound. The **login** OTP counter is server-side/atomic and is NOT affected. | Primary control (random 6-digit code + 10/min rate limit) holds; this is defense-in-depth. Candidate follow-up: move the counter server-side to match the login OTP. |
