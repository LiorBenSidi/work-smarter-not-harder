...
CUE: play the clip (VID_<id1>_<id2>_<id3>.mp4), set to play On click.
...
"It's built to be safe: password hashing, two-step login OTP, email verification, CSRF protection and injection-safe queries. And it's TESTED — all five course test types, each one load-bearing (mutation-tested, not padding), including a real-Mongo integration suite and a cross-container end-to-end test that both GATE the build. A broken wire path can't reach deploy."
