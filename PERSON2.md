...
- [x] **CI/CD deploy pipeline** (PR #91) — the `build` job (docker build `web`+`ai` → GHCR `latest`+`<short-sha>`, `GITHUB_TOKEN` auth) + the `deploy` job (SSH → Azure VM → `docker compose -f docker-compose.prod.yml pull && up -d` → health-curl on the HTTPS FQDN) + `docker-compose.prod.yml` (VM pulls the SHA-pinned images, never builds) + a `Caddyfile` (auto Let's-Encrypt TLS, HTTP→HTTPS). Gated `push && main && vars.SSH_HOST != '' && vars.DEPLOY_ENABLED == 'true'` so `build` runs now and `deploy` stays dormant until the VM exists and the switch is flipped on. **The live half is Elad's:** provision the VM + its GitHub secrets, make the GHCR packages public, set up UptimeRobot, and run the deploy demo (the +10 is graded on that live demo).
